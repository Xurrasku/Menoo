...
import { useRef, useState, useTransition, useEffect, type ChangeEvent } from "react";
...
export function OnboardingMenuStep({ locale, copy, restaurantId, restaurantSlug }: OnboardingMenuStepProps) {
  const router = useRouter();
  const [isPending, startTransition] = useTransition();
  const fileInputRef = useRef<HTMLInputElement | null>(null);
  
  // Restore menu step state from sessionStorage
  const getStoredMenuState = () => {
    if (typeof window === "undefined") return { choice: null };
    try {
      const stored = sessionStorage.getItem("onboarding-menu-state");
      if (stored) {
        return JSON.parse(stored);
      }
    } catch (error) {
      console.error("Failed to parse stored menu state", error);
    }
    return { choice: null };
  };

  const storedState = getStoredMenuState();
  const [choice, setChoice] = useState<"upload" | "example" | null>(storedState.choice);
  ...
  // Save menu step state to sessionStorage whenever it changes
  useEffect(() => {
    if (typeof window !== "undefined") {
      try {
        sessionStorage.setItem("onboarding-menu-state", JSON.stringify({ choice }));
      } catch (error) {
  ...
  async function saveMenu(menuData: {
    ...
    try {
        ...
        try {
          ...
          if (contentType && contentType.includes("application/json")) {
            ...
            if (errorPayload) {
              ...
              } else if (errorPayload.error?.formErrors && Array.isArray(errorPayload.error.formErrors)) {
                errorMessage = errorPayload.error.formErrors.join(" ");
              }
            }
          }
        } catch (parseError) {
      ...
      // Verify the response contains menu data
      const result = await response.json().catch(() => null);

      if (!result || !result.data) {
        throw new Error("Menu was not created successfully");
      }
  ...
  async function handleBack() {
    startTransition(() => {
      router.push(`/${locale}/dashboard/design/onboarding` as Route);
    });
  }
      ...
      {!choice && (
        ...
        >
          <motion.button
            ...
            className="group relative flex flex-col items-center gap-4 rounded-2xl p-8 text-center transition-all duration-300 bg-gradient-to-br from-primary/5 to-violet-50 hover:from-primary/10 hover:to-violet-100/50 ring-1 ring-primary/20 hover:ring-primary/40 shadow-sm hover:shadow-md"
          ...
          <motion.button
            ...
            className="group relative flex flex-col items-center gap-4 rounded-2xl p-8 text-center transition-all duration-300 bg-slate-50/50 hover:bg-slate-100/50 ring-1 ring-slate-200/60 hover:ring-slate-300/60 shadow-sm hover:shadow-md"
            whileHover={{ y: -4, scale: 1.01 }}
            whileTap={{ scale: 0.99 }}
          >
        ...
        >
          ...
          {pendingImages.length > 0 ? (
            ...
            <div
              ...
              onClick={() => !isAiGenerating && fileInputRef.current?.click()}
              className={cn(
                "relative flex h-64 w-full cursor-pointer flex-col items-center justify-center rounded-2xl border-2 border-dashed transition-all shadow-sm",
                isDragging
            ...
            >
              ...
              <div className="flex flex-col items-center gap-4 p-8">
                ...
                <div className="space-y-2 text-center">
                  ...
                  <p className="text-sm text-muted-foreground">{copy.fileTypes}</p>
                </div>
              </div>
            </div>
          )}
